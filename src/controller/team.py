...
def remove_user_from_team(team_id, user_id):
    Team.objects(id=team_id, members__user_id=user_id).update(pull__members__user_id=user_id)
...
def update_user_role(team_id, user_id, role):
    Team.objects(id=team_id, members__user_id=user_id).update_one(set__members__S__role=role)
