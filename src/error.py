...
class NoJsonPayloadException(APIException):
    def __init__(self):
        super(NoJsonPayloadException, self).__init__(
            status_code=400, error_code=1002, message="No JSON data in message body"
        )
...
class MalformedPayloadException(APIException):
    def __init__(self, description):
        super(MalformedPayloadException, self).__init__(
            status_code=400,
            error_code=1003,
            message="The JSON payload was malformed: " + description,
        )


class BadParameterError(APIException):
    def __init__(self, description):
        super(BadParameterError, self).__init__(
            status_code=400, error_code=1005, message="Bad Parameter: " + description
        )
...
class MethodNotAllowedError(APIException):
    def __init__(self):
        super(MethodNotAllowedError, self).__init__(
            status_code=405,
            error_code=1004,
            message="Method not allowed in this context.",
        )
...
class InvalidRequestHeader(APIException):
    def __init__(self):
        super(InvalidRequestHeader, self).__init__(
            status_code=400,
            error_code=1006,
            message="The request header contains invalid or contradicting fields or values.",
        )
