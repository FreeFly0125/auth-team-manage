...
def save_session():
    ...
    if (
        "session" in g
        and g.session is not None
        and g.session["userRole"] != UserRole.APP
    ):
...
class NoAuthorizationHeaderError(APIException):
    def __init__(self):
        super(NoAuthorizationHeaderError, self).__init__(
            status_code=401, error_code=1101, message="No authorization header"
        )
...
class SessionExpiredError(APIException):
    def __init__(self):
        super(SessionExpiredError, self).__init__(
            status_code=403, error_code=1102, message="Your session has expired"
        )
...
class ClientOriginViolation(APIException):
    def __init__(self):
        super(ClientOriginViolation, self).__init__(
            status_code=403,
            error_code=1104,
            message="The request was sent from a new IP address, please login again",
        )
