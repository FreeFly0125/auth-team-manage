...
import auth
from auth import noauth, restrict_to
    ...
    if _check_password(g.payload["password"]):
        if _mail_available(g.payload["mail"]):
            ...
            if "role" in g.payload and g.payload["role"] != "user":
                raise APIException(message="Only normal users can be registered using this path at the moment", status_code=403)
            else:
                u.role = "user"
...
class InvalidCredentialsError(APIException):
    def __init__(self):
        super(InvalidCredentialsError, self).__init__(status_code=403, error_code=1203, message="Userame and password do not match any account")